...
ps_values <- c(
  ...
  "recruit_int_summ", "screen_status"
)
...
count_rct_flow <- function(ps_df, full_list, sites) {
  ...
  ps <- ps_df %>%
    rowwise() %>%
    filter(!is.na(
      sum(elig_project_1, elig_project_2, elig_project_3)
    )) %>%
    ungroup()
  
  # filter unique, indicated by strings greater than 7 characters
  ps_unique <- ps %>%
    filter(nchar(redcap_id) == 7 & substr(redcap_id, 1, 1) == "S")
