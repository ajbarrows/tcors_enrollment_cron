...
download_rc_dataframe <- function(rcon, fields, events) {
  ...
  rc_df <- exportRecords(rcon,
    fields = c("screen_id", fields), labels = FALSE, survey = FALSE,
    dag = TRUE, events = events, form_complete_auto = FALSE
  )
  rc_df <- rc_df %>% filter(!grepl("-2", rc_df$screen_id))
  rc_df <- rc_df %>% filter(!grepl("test", rc_df$screen_id))
  rc_df <- rc_df %>% filter(grepl("X-", rc_df$screen_id) |
    grepl("Y-", rc_df$screen_id) |
    grepl("Z-", rc_df$screen_id))
  rc_df$pi_prop <- ifelse(substr(rc_df$screen_id, 4, 4) == 9, "pilot", "proper")
  # write.csv(df, "s3/rc_df.csv", row.names = FALSE)
  save(rc_df, file = "s3/rc_df.RData")
  rc_df
}
...
download_ps <- function(rcon, fields, events, site, vars_to_keep) {
  ...
  df <- df %>%
    plyr::rename(c(
      ...
      "recruit_1___19" = "tiktok", "recruit_1___20" = "BuildClinical",
    ...
    select(redcap_id, all_of(vars_to_keep), all_of(ps_values), site)
...
pull_prescreen_data <- function(){
  ...
  ##### recode facebook ------
  ps_df_uvm <- recode_facebook(ps_df_uvm, start_date = as.Date("2021-10-22"), end_date = Sys.Date())
  #####
  # straighten things out
  ps_df_brown$BuildClinical <- NA
  ps_df_jhu$BuildClinical <- NA
...
recode_facebook <- function(ps_df, start_date, end_date) {
    ps_df$BuildClinical[ps_df$recruit_date >= start_date & 
                          ps_df$recruit_date <= end_date & 
                          ps_df$facebook == 1] <- 1
    ps_df$facebook[ps_df$recruit_date >= start_date & 
...
est_scrn_sched <- function(rcon) {
  scrn_sched <- exportRecords(rcon, fields = "recruit_date_screen")
...
load_ivr <- function(direc) {
  ...
  pw_file <- read.csv("password.csv", stringsAsFactors = FALSE)
  user <- pw_file$username[1]
  pw <- pw_file$password[1]
...
ivr_timeseries <- function(ivr, sid) {
  df <- ivr %>%
    ...
    mutate(across(.cols = -calldate, .fns = as.numeric))
...
sl_values <- c(
  ...
  "sl_baseline1_dose", "sl_ineligible", "screen_sex",
  "screen_age", "rescreen_int_age", "screen_education"
)
...
ps_values <- c(
  ...
  "recruit_int_summ", "screen_status", "screen_subjectid"
)
...
summarize_prescreen <- function(df, min_date) {
  # aggregate each day, then return cumulative sum
  
  df <- clean_prescreen_df(df, min_date)
  summary <- df %>%
    group_by(site, recruit_date) %>%
    arrange(recruit_date) %>%
    summarize(across(starts_with("elig"), sum, na.rm = TRUE)) %>%
    rename(
    ...
    mutate(across(
      c(project_1, project_2, project_3, ineligible),
      ~cumsum(.x),
      na.rm = TRUE
    ))
  ...
  summary <- merge(totals, summary)

  summary <- summary %>%
    select(
      site, recruit_date, total, project_1,
  ...
  current <- summary %>%
    group_by(site) %>%
    filter(recruit_date == max(recruit_date, na.rm = TRUE)) %>%
    select(-recruit_date)
...
count_rct_flow <- function(ps_df, full_list, sites, min_date) {
  ...
  rct_summary$recruit_int_summ <- factor(
    rct_summary$recruit_int_summ, levels = c(4, 3, 1, 2, 5)
  )
...
ps_inel_one <- function(df_ps_inel, direc) {
  df <- df_ps_inel %>%
    ...
    select(-c(
      ...
      elig_project_none, redcap_id, exclude_gen, screen_subjectid
    )) %>%
    mutate(length_dose = ifelse(length_dose < 30, 1, 0)) %>%
    mutate(five_per_day = ifelse(num_cigs < 5, 1, 0)) %>%
    mutate(p1_age = ifelse(age > 44, 1, 0)) %>%
...
exec_summary <- function(ip_list, current_enrollment, exec_by_site) {
  ...
  df <- df %>%
    mutate(
      experimental = ifelse(is.na(received_product), 0, received_product),
      baseline = in_progress - experimental,
      screened = screening_1_complete + under_lmp_review
      ) %>%
    # rename("screening" = screening_1_complete) %>%
    select(
      project, site, screened, baseline, experimental, complete, pi_prop
    )
  
  # by-site toggle
  if (!exec_by_site) {
    df %>%
      group_by(project, pi_prop) %>%
      summarize(across(.cols = -site, .fns = sum, na.rm = TRUE)) %>%
      ungroup()
...
attrition <- function(current_enrollment, att_by_site) {
  
  #TODO: clean this nonsense up...
  
  c <- current_enrollment %>%
    select(project, site, randomized, withdrawn__postproduct, pi_prop)
  
  if (att_by_site) {
    c <- c %>% group_by(project, site, pi_prop) %>%
      mutate(percent_of_randomized = round(
      withdrawn__postproduct / randomized, digits = 4) * 100
      ) %>%
      ungroup() %>%
      select(
        project, site, randomized, withdrawn__postproduct, 
        percent_of_randomized, pi_prop
      ) 
    colnames(c) <- c(
      "project", "site",
      "randomized",
    ...
    c <- c %>% 
      ...
      ungroup() %>%
      select(
        project, randomized, withdrawn__postproduct, 
        percent_of_randomized, pi_prop
      ) 
    colnames(c) <- c(
      "project",
...
build_sex_table <- function(rc_df, full_subject_list, sex_by_site, sites) {
  
  # limit to randomized only
  rand_df <- full_subject_list %>% filter(is2_date == "randomized")
  
  sex_df_r <- rc_df %>%
    rename("subjectid" = screen_id) %>%
    filter(subjectid %in% rand_df$subjectid) %>%
    pi_prop() %>%
    filter(redcap_event_name == "screening_arm_1") %>%
    mutate(screen_age = ifelse(!is.na(rescreen_int_age), rescreen_int_age, screen_age)) %>%
    select(project = screen_project, 
           site = redcap_data_access_group,
           screen_sex, screen_age, screen_education,
           pi_prop
    ) %>%
    filter(site %in% sites)
  ...
  tab <- sex_df %>%
    tidyr::pivot_wider(
      names_from = screen_sex,
      values_from = n
    ) %>%
    mutate(
      `% Female` = round(Female / sum(Female, Male, na.rm = TRUE), 4) * 100
    )
...
p1_age <- function(sex_df) {
  # mutate(p1_age = ifelse(screen_age < 45, TRUE, FALSE)) %>%
  sex_df %>%
    mutate(
      screen_education = redcapFactorFlip(screen_education),
      p1_ed = ifelse(screen_education < 17, TRUE, FALSE),
      p1_age = ifelse(screen_age < 45, TRUE, FALSE),
      p1_population = ifelse(p1_ed & p1_age, "yes", "no")
      ) %>%
    group_by(project, pi_prop, p1_population) %>%
    count() %>%
    tidyr::pivot_wider(
      names_from = p1_population,
      values_from = n
    ) %>%
    mutate(`% P1-Like` = round(yes / sum(yes, no, na.rm = TRUE), 4) * 100)
}
...
load_prescreen_history <- function(direc) {
  filename <- paste(direc, "prescreen_history.csv", sep = "")
...
assemble_rct_source <- function(ps_df, source_vars, min_date) {
  ...
  ps_df %>%
    select(
      site, recruit_date, all_of(source_vars), screen_status,
      elig_project_1, elig_project_2, elig_project_3) %>%
    mutate(
      ...
      project = ifelse(
        elig_project_1 == 1 & elig_project_3 == 1,
        "Projects 1 and 3", "none"
      ),
      project = ifelse(
        elig_project_1 == 1 & elig_project_3 == 0, "Project 1", project),
      project = ifelse(elig_project_2 == 1, "Project 2", project),
      project = ifelse(
        elig_project_3 == 1 & elig_project_1 == 0, "Project 3", project
      )) %>%
    select(-c(elig_project_1, elig_project_2, elig_project_3)) %>%
    filter(recruit_date < Sys.Date() & recruit_date > min_date) %>%
    group_by(recruit_date, site, screen_status, project) %>%
    summarize_all(~ sum(., na.rm = TRUE)) %>%
    tidyr::pivot_longer(cols = -c(
      recruit_date, site, screen_status, project
    )) %>%
    group_by(site, name) %>%
...
rct_source_table <- function(rct_source) {
  rct_source %>%
    group_by(screen_status, project, name) %>%
    summarize(count = sum(value, na.rm = TRUE)) %>%
    tidyr::pivot_wider(
      names_prefix = "screened_",
      names_from = screen_status,
      values_from = count
    ) %>%
    rename("screened" = `screened_TRUE`,
           "not_screened"= `screened_FALSE`,
           "source" = name) %>%
    mutate(total_prescreens = screened + not_screened) %>%
    arrange(-total_prescreens) %>%
    select(source, total_prescreens, everything()) %>%
    mutate(
      retention = paste(round((
        screened / total_prescreens
        ) * 100, 2), "%", sep = "")) %>%
    mutate(
      screened = paste(
        screened, "/", total_prescreens, " (", retention, ")", sep = ""
        )
    ) %>%
    select(-c(total_prescreens, not_screened, retention)) %>%
    tidyr::pivot_wider(
      names_from = project, 
      values_from = screened
    ) %>%
    select(source, `Project 1`, `Project 2`, `Project 3`, `Projects 1 and 3`)
}

rct_source_alluvial <- function(rc_df, ps_df, show_screened) {
  
  rc <- rc_df %>% 
    filter(redcap_event_name == "screening_arm_1") %>%
    select(screen_id, screen_project, sl_status)
  ps <- ps_df %>% select(all_of(source_vars), screen_subjectid, site)
  
  df <- rc %>% 
    full_join(ps, by = c("screen_id" = "screen_subjectid")) %>%
    # filter(nchar(screen_id) == 6) %>%
    mutate(screened = ifelse(!is.na(screen_project), "screened", "not_screened")) %>%
    select(-c(screen_id, sl_status)) %>%
    tidyr::pivot_longer(
      cols = -c(screen_project, screened, site),
      names_to = "source") %>%
    group_by(screen_project, source, screened, site) %>%
    summarize(number_screened = sum(value, na.rm = TRUE)) %>%
    arrange(-number_screened) %>%
    filter(number_screened > 0)
  
  if (!show_screened) {
    df_plot <- df %>%
      mutate(screen_project = ifelse(is.na(screen_project), "not_screened", screen_project))
  } else {
    df_plot <- df %>%
      filter(!is.na(screen_project))
  }
  
  ggplot(df_plot, aes(y = number_screened, axis1 = source, axis2 = screen_project)) +
    geom_alluvium(aes(fill = source), show.legend = FALSE, na.rm = TRUE) +
    geom_stratum(width = 3/12, alpha = .01) +
    geom_label(stat = "stratum",
               aes(label = after_stat(stratum)),
               size = 5) +
    scale_x_discrete(limits = c("source", "screen_project"), expand = c(0, -.75)) +
    # scale_fill_brewer(palette = "Paired") +
    labs(title = "Screenings Attended by Source",
         y = "participants")
}
...
ivr_summary <- function(ivr) {
  ivr %>%
    ...
    summarise(
      studycigs = mean(as.numeric(studycigs), na.rm = TRUE),
      nonstudycigs = mean(as.numeric(nonstudycigs), na.rm = TRUE)
    ) %>%
...
# plotting functions -------------
# df <- enrollment_history
plot_enrollment <- function(enrollment_history, timeframe, israte = FALSE) {
  ...
  df <- enrollment_history %>%
    ...
    select(-c(under_lmp_review, in_progress, complete)) %>%
  ...
  if (timeframe == "1 month") {
    df_sum <- df_sum %>% filter(date >= Sys.Date() - 30)
  } else if (timeframe == "3 months") {
    df_sum <- df_sum %>% filter(date >= Sys.Date() - 90)
  } else if (timeframe == "1 year") {
    df_sum <- df_sum %>% filter(date >= Sys.Date() - 365)
  }
  
  if (israte) {
    df_sum <- df_sum %>% 
      group_by(project, variable, pi_prop) %>%
      mutate(value = value - lag(value)) %>%
      tidyr::complete(date = seq.Date(min(date), max(date), by = "day")) %>%
      mutate(
        value = ifelse(is.na(value), 0, value),
        value = zoo::rollmean(value, k = 7, fill = NA))
  }
  
  ggplotly(ggplot(df_sum, aes(x = date, y = value), width = 800, height = 600) +
...
plot_prescreen <- function(ps_history, inc_comp_date, checkCompDate, date_range) {
  ...
  df$site <- factor(df$site, levels = c("uvm", "brown", "jhu"))
  p <- ggplot(df, aes(x = recruit_date, y = value)) +
...
get_session_dates <- function(rc_df) {
  ...
  # last session list, as well as CO tracking. if there's a rescreen,
  # use that instead of screening
  session_dates <- rc_df %>%
    select(
    ...
    ungroup() %>%
    mutate(screen_date = ifelse(!is.na(rescreen_date), as.character(rescreen_date), as.character(screen_date))) %>%
    mutate(across(contains("date"), ymd)) %>%
    tidyr::unite(col = "date", -c(
      screen_id, redcap_event_name, sl_condition, sl_ecig_condition, rescreen_date,
...
plot_randomization_goal <- function(enrollment_history, min_date, max_date,
  ...
  goal_lab <- paste("Goal = ", max_enroll)
  ...
  p <- ggplot(enrollment_goal, aes(date, target)) +
    ...
    theme(legend.position = "top")
  p +   annotate(
    "text", x = min_date + 150, y = max_enroll + 5, label = goal_lab
  )
}
